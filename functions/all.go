...
func allIterator(c eval.EvalContext, arg eval.IterableValue, block eval.Lambda) eval.PValue {
	return types.WrapBoolean(arg.Iterator().All(func(v eval.PValue) bool { return eval.IsTruthy(block.Call(c, nil, v)) }))
}

func allIndexIterator(c eval.EvalContext, iter eval.IterableValue, block eval.Lambda) eval.PValue {
	index := int64(-1)
...
func allHashIterator(c eval.EvalContext, iter eval.IterableValue, block eval.Lambda) eval.PValue {
	return types.WrapBoolean(iter.Iterator().All(func(v eval.PValue) bool {
