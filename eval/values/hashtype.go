...
func NewHashType2(args ...PValue) *HashType {
	...
	if !ok {
		panic(NewIllegalArgumentType2(`Hash[]`, 0, `Type`, args[0]))
	}
	...
	case 3:
		...
		if rng, ok = sizeArg.(*IntegerType); !ok {
			...
			if sz, ok = toInt(sizeArg); !ok {
				panic(NewIllegalArgumentType2(`Hash[]`, 2, `Integer or Type[Integer]`, args[2]))
			}
	...
	case 4:
		...
		if min, ok = toInt(args[2]); !ok {
			panic(NewIllegalArgumentType2(`Hash[]`, 2, `Integer`, args[2]))
		}
		if max, ok = toInt(args[3]); !ok {
			panic(NewIllegalArgumentType2(`Hash[]`, 3, `Integer`, args[3]))
		}
...
func (t *HashType) ToString(bld Writer, format FormatContext, g RDetect) {
	WriteString(bld, `Hash`)
	...
	WriteByte(bld, '[')
	writeRange(bld, t.size, writeTypes(bld, format, g, t.keyType, t.valueType), true)
	WriteByte(bld, ']')
}
