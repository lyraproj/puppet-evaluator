...
func (t *FloatType) ToString(bld Writer, format FormatContext, g RDetect) {
	WriteString(bld, `Float`)
	if !t.IsUnbounded() {
		WriteByte(bld, '[')
		if t.min == -MaxFloat64 {
			WriteString(bld, `default`)
		} else {
			WriteString(bld, pn.LiteralPN(t.min).String())
		}
		WriteByte(bld, ',')
		if t.max == MaxFloat64 {
			WriteString(bld, `default`)
		} else {
			WriteString(bld, pn.LiteralPN(t.max).String())
		}
		WriteByte(bld, ']')
	}
}
...
func (fv *FloatValue) ToKey() HashKey {
	return HashKey(fmt.Sprintf("\x01f%v", fv.Float()))
}
