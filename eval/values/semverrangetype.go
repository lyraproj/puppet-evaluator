...
func (t *SemVerRangeType) ToString(bld Writer, format FormatContext, g RDetect) {
	WriteString(bld, `SemVerRange`)
}
...
func (bv *SemVerRangeValue) ToKey() HashKey {
	b := bytes.NewBufferString("\x01R")
	bv.ToString(b, NONE, nil)
