...
func (t *StructType) ToString(bld Writer, format FormatContext, g RDetect) {
	WriteString(bld, `Struct`)
	top := len(t.elements)
	if top == 0 {
		return
	}
	WriteString(bld, `[{`)
	t.elements[0].ToString(bld, format, g)
	for idx := 1; idx < top; idx++ {
		WriteByte(bld, ',')
		t.elements[idx].ToString(bld, format, g)
	}
	WriteString(bld, `}]`)
}
