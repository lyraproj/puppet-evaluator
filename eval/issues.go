...
func init() {
	...
	HardIssue(EVAL_CTOR_NOT_FOUND, `Unable to load the constructor for data type '%{type}'`)
	...
	HardIssue(EVAL_SERIALIZATION_DEFAULT_CONVERTED_TO_STRING, `%{path} contains the special value default. It will be converted to the String 'default'`)

	HardIssue(EVAL_SERIALIZATION_ENDLESS_RECURSION, `Endless recursion detected when attempting to serialize value of class %{type_name}'`)

	HardIssue2(EVAL_SERIALIZATION_UNKNOWN_CONVERTED_TO_STRING, `%{path} contains %{klass} value. It will be converted to the String '%{value}'`, HF{`klass`: A_an})
